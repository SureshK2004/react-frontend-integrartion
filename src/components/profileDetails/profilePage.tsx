...
const ProfilePage: React.FC = () => {
  ...
  const iconConfig = {
    ...
    calendar: { icon: Calendar, color: "text-purple-500 bg-purple-50 dark:bg-purple-900/20" },
    ...
    building: { icon: Building, color: "text-indigo-500 bg-indigo-50 dark:bg-indigo-900/20" },
    briefcase: { icon: Briefcase, color: "text-amber-500 bg-amber-50 dark:bg-amber-900/20" },
  };

  const IconWrapper = ({ type, children }: { type: keyof typeof iconConfig; children: React.ReactNode }) => {
    const config = iconConfig[type];
    const IconComponent = config.icon;
    
    return (
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-xl ${config.color} transition-all duration-300 hover:scale-110`}>
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex justify-end mb-4">
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-primary to-purple-600 text-white rounded-xl hover:from-primary/90 hover:to-purple-600/90 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 text-sm"
        >
