...
const Designation: React.FC = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(5); // Changed from fixed to state
  ...
  const fetchDesignations = useCallback(async () => {
    try {
      ...
      if (response.status === "success") {
        ...
        // Set pagination values from API
  ...
  }, [page, itemsPerPage]); // Added itemsPerPage dependency

  // Trigger fetching
  useEffect(() => {
    fetchDesignations();
  }, [page, itemsPerPage, refresh, fetchDesignations]); // Added itemsPerPage dependency
